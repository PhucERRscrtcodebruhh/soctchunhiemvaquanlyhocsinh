...
export function BienBanBanGiao({ maLop, classData }) {
  ...
  return (
    <ModuleContainer title="BIÊN BẢN BÀN GIAO NGHỈ TẾT & SINH HOẠT HÈ" desc="Soạn thảo trực tiếp Word (.docx) • Nhập/Xuất văn bản • Quản lý hồ sơ">
      {/* View Switcher */}
      <div className="flex items-center gap-2 p-1.5 bg-slate-950/80 border border-slate-800 rounded-xl mb-4 w-fit">
        <button
        ...
        >
          ...
          <span>Danh sách bàn giao ({list.length})</span>
        </button>
      </div>

      {viewMode === 'editor' ? (
        <WordDocumentEditor
          defaultFileName={`Bien_Ban_Ban_Giao_${currentLop}`}
          maLop={currentLop}
          title={`BIÊN BẢN BÀN GIAO NGHỈ TẾT & HÈ - LỚP ${currentLop}`}
        />
      ) : (
        <>
          <div className="flex flex-wrap items-center justify-between gap-3 p-4 bg-slate-950/60 border border-slate-800 rounded-xl mb-4">
